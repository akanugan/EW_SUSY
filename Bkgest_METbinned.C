...
void Bkgest_METbinned(){
  ...
  file_inputdistributions = new TFile("MET_binned/Total_MC2018.root", "READ");
  ...
  TH1F * h_Bkg1 = (TH1F*) file_inputdistributions->Get("WHAK8J2Mass2");
  TH1F * h_Bkg2 = (TH1F*) file_inputdistributions->Get("WHAK8J2MassNo2bTag2");
  TH1F * h_Bkg3 = (TH1F*) file_inputdistributions->Get("HWAK8J1Mass2");
  TH1F * h_Bkg4 = (TH1F*) file_inputdistributions->Get("HWAK8J1MassNo2bTag2");
  ...
  Double_t R1_error =0, R2_error =0, R3_error =0, P1_error =0, P2_error =0, P3_error =0, B1_avg=0, D_avg=0, A1_avg = 0, C_avg=0, kappa =0;
  ...
  B1_avg = (Bkg1_A_T+Bkg1_C_T)/2;
  cout<<"B avg "<<B1_avg<<endl;
  D_avg = (Bkg2_A_T+Bkg2_C_T)/2;
  cout<<"D avg "<<D_avg<<endl;
  A1_avg = Bkg1_B_T;
  C_avg = Bkg2_B_T;
  kappa = (B1_avg)*(C_avg)/((A1_avg)*(D_avg));
  ...
  auto c1 = new TCanvas("c1","Ratio pass/fail plots",200,10,700,500);
  //c1->SetFillColor(42);
  c1->SetGrid();
  ...
  auto c2 = new TCanvas("c2","kappa plot",200,10,700,500);
  //c1->SetFillColor(42);
  c2->SetGrid();
  //c1->GetFrame()->SetFillColor(21);
  c2->GetFrame()->SetBorderSize(12);
  const Int_t m = 3;
  Double_t xbin[m+1]  = {300.,500.,700.,2000.};
  Double_t x2[n]  = {(xbin[0]+xbin[1])/2,(xbin[1]+xbin[2])/2,(xbin[2]+xbin[3])/2};
  //Double_t x[n]  = {67.5,110.,192.5};
  
  Double_t y2[n]  = {0.966,0.375,1.239};
  //  Double_t ex[n] = {17.5,25.,57.5};
  Double_t ex2[n] = {(xbin[1]-xbin[0])/2,(xbin[2]-xbin[1])/2,(xbin[3]-xbin[2])/2};
  Double_t ey2[n] = {0,0,0};
  auto gr2 = new TGraphErrors(m,x2,y2,ex2,ey2);

  gr2->SetTitle(filename);
  gr2->GetYaxis()->SetTitle("Kappa"); 
