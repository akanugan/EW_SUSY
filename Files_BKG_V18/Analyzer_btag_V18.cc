	...
	if((*JetsAK8)[i].Pt() > 200 && abs((*JetsAK8)[i].Eta()) < 2.0){
	  ...
	  if(jetsAK8hasb[i]){ //FOR Z CAND.
	    if(
	       ((*JetsAK8_softDropMass)[i] > massLowZ) &&
	       ((*JetsAK8_softDropMass)[i] < massHighZ)) {
	      if( ((*JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb)[i] > deepbbscore) ) { 
		Zcand = true;
		Zcand_SR_cnt +=1;
	      }
	      else{
...
void SignalReg::print(Long64_t jentry){
  ...
  for(int i=0;i<JetsAK8->size();i++){
    ...
    cout<<"AK8 pT:"<<(*JetsAK8)[i].Pt()<<" eta:"<<(*JetsAK8)[i].Eta()<<" phi:"<<(*JetsAK8)[i].Phi()<<" softDropM:"<<(*JetsAK8_softDropMass)[i]<<" tau21:"<<(*JetsAK8_NsubjettinessTau2)[i]/(*JetsAK8_NsubjettinessTau1)[i]<<" DeepDoubleBDiscr:"<<(*JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb)[i]<<" WDiscr:"<<(*JetsAK8_DeepTagWvsQCD)[i]<<endl;
  }
}
