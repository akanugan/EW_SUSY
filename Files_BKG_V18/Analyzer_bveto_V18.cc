...
void SignalReg::EventLoop(const char *data,const char *inputFileList) {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(!isMC){
      ...
      // }
	if( (*TriggerPass)[110] ==1 || (*TriggerPass)[112]==1 || (*TriggerPass)[116]==1 ||(*TriggerPass)[118]==1 || (*TriggerPass)[120]==1 || (*TriggerPass)[124] ==1) trgPass = true;
      }
    ...
    //cout<<"jets disc size "<<JetsAK8_DeepTagZvsQCD->size()<<endl;
    //cout<<"\n";
    ...
    if (Bkgest_MET){
	 ...
	 ((*JetsAK8_DeepTagWvsQCD)[0] > deepAK8Wscore) )
	{
	  if(         //middle band
	 ...
	 ((*JetsAK8_DeepTagWvsQCD)[1] > deepAK8Wscore) )
	{
	...
	if( ((*JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb)[1] > deepbbscore) ){ // Sig reg mass range
	  h_WHAK8J1MassSB->Fill((*JetsAK8_softDropMass)[0],wt); 
	  h_WHAK8J2MassSB->Fill((*JetsAK8_softDropMass)[1],wt); 
	}
	  
	//if( ((*JetsAK8_doubleBDiscriminator)[1] < bbscore) ) {
	if( ((*JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb)[1] < deepbbscore) ){ // 2b side band 
	...
	if( ((*JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb)[0] > deepbbscore) ){
    ...
    if(WH_SR){
	 ...
	 ((*JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb)[1] > deepbbscore) ) {      
	...
	h_WHAK8J1wDis->Fill(((*JetsAK8_DeepTagWvsQCD)[0]),wt);
	...
	h_WHAK8J2wDis->Fill(((*JetsAK8_DeepTagWvsQCD)[1]),wt);
	...
	h2_WHmtbminHmass->Fill(mtbmin,(*JetsAK8_softDropMass)[1],wt);
	 ...
	 ((*JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb)[0] > deepbbscore) ){
	...
	h_HWAK8J1wDis->Fill(((*JetsAK8_DeepTagWvsQCD)[0]),wt);
	...
	h_HWAK8J2wDis->Fill(((*JetsAK8_DeepTagWvsQCD)[1]),wt);
	...
	if((*JetsAK8)[i].Pt() > 200 && abs((*JetsAK8)[i].Eta()) < 2.0){
	  if(jetsAK8hasb[i]){ //FOR Z OR H CAND.
	    ...
	    if(
	       ...
	       ((*JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb)[i] > deepbbscore) ) { 
	      ZorHcand = true;
	    }
	  }
	  else{ //W cand
	    ...
	    if(
	       ...
	       ((*JetsAK8_DeepTagWvsQCD)[i] > deepAK8Wscore) ){
	      Wcand = true;
	    }
	  }
	}
	...
	if(
	  ...
	  if( (*JetsAK8_DeepTagWvsQCD)[w] > 0.918 ){
	    Wdis.push_back((*JetsAK8_DeepTagWvsQCD)[w]);
	    Wdismax = *max_element(Wdis.begin(), Wdis.end());  //WDis with highest value passing the cut
	  }
	  if((*JetsAK8_DeepMassDecorrelTagWvsQCD)[w] > 0.704 ){
	    WdisMD.push_back((*JetsAK8_DeepMassDecorrelTagWvsQCD)[w]);
      ...
      for(int i=0; i < JetsAK8->size(); i++){ // W
      	if ( Wdismax > 0 &&
	     //	     (*JetsAK8_DeepTagWvsQCD)[i] == Wdismax &&  // For max W-tag
	     abs( (*JetsAK8_DeepTagWvsQCD)[i] - Wdismax ) < 1e-6 &&  // For max W-tag
	     //(*JetsAK8_DeepMassDecorrelTagWvsQCD)[i] == WdisMDmax &&
	...
	//if( (*JetsAK8_DeepTagWvsQCD)[j] > 0.918 || (*JetsAK8_DeepTagZvsQCD)[j] > 0.918 ){
	if( (j != widx) &&
	    // (*JetsAK8_DeepMassDecorrelTagWvsQCD)[j] > 0.704 &&
	  ...
	  if( (((*JetsAK8_NsubjettinessTau2)[j]/(*JetsAK8_NsubjettinessTau1)[j]) > 0.45) ){
	    twoT2MCR = true;
	  }
	  
	  if( (((*JetsAK8_NsubjettinessTau2)[j]/(*JetsAK8_NsubjettinessTau1)[j]) > 0.35) && (((*JetsAK8_NsubjettinessTau2)[j]/(*JetsAK8_NsubjettinessTau1)[j]) < 0.45) ){
	    oneT2M = true;
	  }
	}
      }
	...
	if(
	   ((*JetsAK8_DeepTagWvsQCD)[0] > 0.918) &&  // 1% mistag 
	   ((*JetsAK8_DeepTagWvsQCD)[1] > 0.918) ) { 
	  W_disc1 = MAX((*JetsAK8_DeepTagWvsQCD)[0],(*JetsAK8_DeepTagWvsQCD)[1]);
	  cout<<"Wdis1"<<endl;
	...
	if(
	   ((*JetsAK8_DeepTagWvsQCD)[0] > 0.961) &&   // 0.5% mistag 
	   ((*JetsAK8_DeepTagWvsQCD)[1] > 0.961) ) { 
	  W_disc2 = MAX((*JetsAK8_DeepTagWvsQCD)[0],(*JetsAK8_DeepTagWvsQCD)[1]);
	   cout<<"Wdis2"<<endl;
	...
	if(
	   ((*JetsAK8_DeepTagWvsQCD)[1] ==  high_W_disc) &&   // highest W in above 2 cases 
	   ((*JetsAK8_NsubjettinessTau2)[0]/(*JetsAK8_NsubjettinessTau1)[0]) < 0.35 ){
    ...
    if (WorZ_genrecomatch){
	 ...
	 ((*JetsAK8_softDropMass)[0] > 75 && (*JetsAK8_softDropMass)[0] < 100) ){ 
		...
		h_wAK8J1zDis->Fill(((*JetsAK8_DeepTagZvsQCD)[1]),wt);
		h_wAK8J1wDis->Fill(((*JetsAK8_DeepTagWvsQCD)[1]),wt);
		h_wAK8J1wDisDC->Fill(((*JetsAK8_DeepMassDecorrelTagWvsQCD)[1]),wt);
	      }
	    }
		...
		h_zAK8J1zDis->Fill(((*JetsAK8_DeepTagZvsQCD)[0]),wt);
		h_zAK8J1wDis->Fill(((*JetsAK8_DeepTagWvsQCD)[0]),wt);
		h_zAK8J1wDisDC->Fill(((*JetsAK8_DeepMassDecorrelTagWvsQCD)[0]),wt);
	      }
	    }
	  } //ucsd
	} //gen loop end
    ...
    if(wDisTweak){
	  ...
	  (*JetsAK8_doubleBDiscriminator)[1] >bbscore  ){
	if( (*JetsAK8_DeepTagWvsQCD)[0] > 0.779) {
	  h_WHMETa->Fill(MET,wt);
	}
	if( (*JetsAK8_DeepTagWvsQCD)[0] > 0.981) {
	  ...
	  (*JetsAK8_doubleBDiscriminator)[0] > bbscore)  {
	if( (*JetsAK8_DeepTagWvsQCD)[1] > 0.779) {
	  h_HWMETa->Fill(MET,wt);
	}
	if( (*JetsAK8_DeepTagWvsQCD)[1] > 0.981) {
 	...
 	for (int gen=0; gen < GenParticles_PdgId->size(); gen++){
	  ...
	  if((*GenParticles_PdgId)[gen] == 5 && abs((*GenParticles_ParentId)[gen]) == 25){ //Z->bb //removing double count by having only 1 b and its parrent Z
	    ...
	    if(abs(DeltaR((*GenParticles)[GenZIdx].Eta(),(*GenParticles)[GenZIdx].Phi(),(*JetsAK8)[0].Eta(),(*JetsAK8)[0].Phi())) < 0.1){ //1st ak8 Zbb match
	      ...
	      h_AK8J1deepdoubleBDis->Fill(((*JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb)[0]),wt);
	      h_AK8J1Tau21->Fill(((*JetsAK8_NsubjettinessTau2)[0])/((*JetsAK8_NsubjettinessTau1)[0]),wt);
	      h_AK8J1zDis->Fill(((*JetsAK8_DeepTagZvsQCD)[0]),wt);
	    ...
	    if(abs(DeltaR((*GenParticles)[GenZIdx].Eta(),(*GenParticles)[GenZIdx].Phi(),(*JetsAK8)[1].Eta(),(*JetsAK8)[1].Phi())) < 0.1){ //2nd ak8 Zbb match
	      ...
	      h_AK8J2deepdoubleBDis->Fill(((*JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb)[1]),wt);  
	      h_AK8J2Tau21->Fill(((*JetsAK8_NsubjettinessTau2)[1])/((*JetsAK8_NsubjettinessTau1)[1]),wt);
	      h_AK8J2zDis->Fill(((*JetsAK8_DeepTagZvsQCD)[1]),wt);
	    }
	  } // end 2nd Zbb match
	} // end for
...
void SignalReg::print(Long64_t jentry){
  ...
  for(int i=0;i<JetsAK8->size();i++){
    ...
    cout<<"AK8 pT:"<<(*JetsAK8)[i].Pt()<<" eta:"<<(*JetsAK8)[i].Eta()<<" phi:"<<(*JetsAK8)[i].Phi()<<" softDropM:"<<(*JetsAK8_softDropMass)[i]<<" tau21:"<<(*JetsAK8_NsubjettinessTau2)[i]/(*JetsAK8_NsubjettinessTau1)[i]<<" DeepDoubleBDiscr:"<<(*JetsAK8_pfMassIndependentDeepDoubleBvLJetTagsProbHbb)[i]<<" WDiscr:"<<(*JetsAK8_DeepTagWvsQCD)[i]<<" WdiscrDecorr:"<<(*JetsAK8_DeepMassDecorrelTagWvsQCD)[i]<<endl;
  }
}
