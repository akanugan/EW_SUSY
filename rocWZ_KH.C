...
void rocWZ_KH(){
  ...
  TFile * file_output;
  TString filename = "ROCplot_WinT2ttfastdR_QCD_LDPBkg_2016MC";     //file name to be saved
  TString name;

  file_inputdistributions1 = new TFile("T2tt_MC2016_fastsim.root", "READ"); //sig
  file_inputdistributions2 = new TFile("ROC_Wmatch_v3/Bkg/QCD_HT_LDP_MC2016.root", "READ"); //bkg
  // TTJets_MC2018.root          WJetsToLNu_HT_MC2018.root   ZJetsToNuNu_HT_MC2018.root
  //TChiWZ_600_100_MC2018.root TTJets_MC2018.root
  ...
  */
  ...
  mg->Draw("ALP");
  mg->SetTitle("ROC curve for W(2016 MC)  Sig: T2ttfast_dR(b,AK8) Bkg: QCD_LDP");
  mg->GetXaxis()->SetTitle("Bkg selection");
  mg->GetYaxis()->SetTitle("Signal selection");
  //  gPad->SetLogx();
  auto legend = new TLegend(0.45,0.4,0.9,0.65);
  //  legend->AddEntry(roc_graph1,"AK8J1 Deep Z Disc.","l");
  legend->AddEntry(roc_graph,"AK8 Tau21","l");
  legend->AddEntry(roc_graph2,"AK8 W Disc.","l");
  legend->AddEntry(roc_graph3,"AK8 W Disc. MD","l");
